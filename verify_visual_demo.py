...
try:
    with sync_playwright() as p:
        ...
        page.route(
            "**/api/context?*",
            lambda r: r.fulfill(status=503, body="{}", content_type="application/json"),
        )
        page.route(
            "**/api/webcams", lambda r: r.fulfill(json={"cameras": [], "sources": []})
        )
        ...
        page.wait_for_function(
            "document.querySelector('#evidence-satellite img')?.naturalWidth > 0 && document.querySelector('#evidence-camera img')?.naturalWidth > 0",
            timeout=60000,
        )
        ...
        assert any(
            a["resource"]["kind"] == "helicopter" for a in state["assignments"].values()
        )
        print(
            json.dumps(
                {
                    "vehicles": len(state["assignments"]),
                    "fires": len(state["scenario"]["incidents"]),
                    "images": page.locator("#agent-evidence img").count(),
                    "zoom": page.evaluate("window.__directorMap.getZoom()"),
                    "errors": errors,
                }
            )
        )
