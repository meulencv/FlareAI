...
Prepara el entorno virtual, pide la clave de HappyRobot sin guardarla y abre `http://127.0.0.1:8790`. El modo principal usa los workflows remotos ya desplegados: el arranque NO crea ni republica workflows automáticamente y NO carga YOLO. Enter sin clave o `--local` prepara el modelo/muestras para el laboratorio local opcional. `--no-browser` evita abrir el navegador. No lanzar otra instancia mientras el puerto esté ocupado.
...
## Flujo integrado por incidente y reloj en imagen

- La pantalla principal ahora es REMOTA: ubicación manual o incidente del mapa → catálogo → IA selecciona hasta dos cámaras → visión y reloj → tarjetas con antigüedad y caducidad. El laboratorio YOLO anterior queda plegado, no se ha eliminado. `launch.py` no redespliega el workflow antiguo y el detector local se carga de forma perezosa.
- Orquestador publicado: **FlareAI · Tráfico · Selección por incidente**, workflow `01a0b976-ebaa-74b9-92fd-0f5802278c18`, slug `o3juz8jupuw1`, versión `01a0b976-ebb7-7577-ac4d-0a4cab13ce1d`. Estado sin secretos en `runtime/incident-workflow-state.json`.
- Visión temporal: mismo workflow Vercel `01a0b92b-35cd-78af-a37d-19bf9dce95c3`, nueva versión `01a0b974-548e-796d-a009-dc8f866df3c2`. Conserva la versión anterior. Parámetros: camera_id, image_url, time_zone. Modelo GPT-4.1 mini; cuenta y transcribe la fecha/hora impresa, con abstención si no es legible.
- `incident_workflow.py`: incrusta en el nodo Python una copia comprimida de 2.600 cámaras del catálogo fuente, consulta Haversine con radio 1–30 km y máximo 12 candidatos, y AI Extract elige ≤2 IDs. Los IDs repetidos/inventados o selecciones mayores de dos se rechazan. NO es SQL remoto ni sincronización automática: es un índice de solo lectura copiado a HappyRobot; la UI muestra la fecha de copia.
- El bucle de colección no resolvía su expresión y saltaba la visión. La versión comprobada usa un grupo de una iteración y DOS llamadas acotadas con campos first/second; los slots vacíos se rechazan en el hijo antes de llamar a Vercel. No cambiar a iteraciones arbitrarias. Call Workflow síncrono ejecutaba el hijo, pero devolvía child_workflow_failed por falta de un response node; se cambió a fire_and_forget y se leen los resultados mediante child_run_id. El lector verifica el workflow del hijo y espera su finalización, aunque el padre ya esté completed.
- `recency.py`: fecha completa y segundos, formatos DD-MM-YYYY/DD/MM/YYYY/YYYY-MM-DD; Europe/Madrid o Atlantic/Canary según catálogo. Rechaza fechas imposibles, horas ambiguas/inexistentes de cambio horario, reloj futuro y ausencia de fecha. Máximo 600 segundos; un reloj futuro al analizar no se vuelve válido por esperar. El sandbox tenía zoneinfo pero no datos IANA: se incrustan únicamente los TZIF de Madrid/Canarias desde tzdata==2025.2, sin instalar nada en HappyRobot.
- Hora extraída NO significa fecha certificada. `capture_time_source=image_overlay_ai_transcription` y `capture_time_verified=false` siempre. Se observó una lectura errónea 15/09 en M-30 y se descartó como antigua; no se corrige adivinando que es hoy. La densidad/conteo actual son unknown/null si caduca o el reloj no sirve; `visual_estimate` conserva lo observado para auditoría, no para tratarlo como vigente. `expires_at` es inmutable: la consulta o el análisis no rejuvenecen el fotograma.
- `monitor.py`: coordina la UI con los runs remotos y sus hijos, revalida la vigencia al consultar/exportar, conserva consultas e imágenes por sesión y permite recuperar consultas propias guardadas validando UUID y workflow. `POST /api/remote/start`, `GET /api/remote/job?id=...`, `/api/remote/status`, `/api/remote/image?job=...&camera=...`, `/api/remote/incidents`. No admite URLs libres ni sirve archivos arbitrarios.
- Integración con el mapa: consulta GET http://127.0.0.1:8090/api/data en SOLO LECTURA; la lista se refresca cada minuto mientras la pestaña está visible. No importa módulos del padre ni abre/escribe PostgreSQL o emergencias_espana.db. En este equipo el mapa no respondía en 8090 durante la prueba; la ubicación manual funciona. Si el mapa está en otra máquina, hace falta configurar explícitamente una conexión autorizada; no afirmar que el SQL local ya sea accesible desde la nube.
- `budget.py`: SQLite propio bajo runtime, no la BD original. Tope LOCAL de pruebas de 2 USD Vercel, reserva conservadora de 0,45 USD por imagen (máximo dos), conciliación con recibos numéricos; los importes desconocidos no se liberan sin evidencia. Las conciliaciones quedan registradas. Incluye 0,0019968 USD de pruebas iniciales. No cambia el límite de la clave en Vercel y no controla llamadas realizadas fuera de esta interfaz. No activar barridos nacionales ni cron. El checkbox de seguimiento está apagado por defecto; si el usuario lo activa, repite cada 3 minutos solo con la pestaña visible y presupuesto suficiente.
- La imagen de las tarjetas es una vista del proveedor y puede haber cambiado respecto al fotograma leído por Vercel. Se advierte en pantalla; no se afirma disponer de un archivo inmutable del fotograma analizado. No hay velocidades, bloqueos, rutas ni despachos autorizados.
- Verificación: 69 pruebas Python y 5 JS; `verify_monitor_browser.py --job-id ID` usa Edge headless y NO crea nuevas inferencias. Verifica tarjetas, ausencia de errores JS y caducidad con reloj del navegador simulado. Dependencias en requirements-dev.txt; no instala navegadores globales. `monitor-current.png` es la captura en el momento de la prueba; `monitor-expired-test.png` usa un reloj simulado y no representa un cambio real del tráfico.
- Prueba íntegra: job `d54affb9-b8aa-45a2-9427-66b50c774b27`, run padre `7524ae84-6493-409c-80f7-94bfa6dae41d`; la IA eligió madrid-08301 y madrid-08308 entre 12 candidatos. Hubo una lectura descartada y otra reciente (11 vehículos estimados); el test visual verificó que después caducaba. `verification-monitor.json` conserva resultados/coste, sin claves. El contador local acumulaba 0,005775 USD, con 1,994225 USD restantes. No es un benchmark etiquetado de visión.
